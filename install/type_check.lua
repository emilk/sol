...
local function analyze(ast, filename, on_require, settings)
	...
	local function check_arguments(expr, fun_t, arg_ts)
		assert(fun_t.args)
		local fun_name = fun_t.name or "lambda"
	...
	local function call_function(expr, scope)
		...
		local function try_call(typ, report_errors)
			...
			elseif typ.tag == 'object' and typ.metatable and typ.metatable.members['__call'] then
				...
				local call_t = typ.metatable.members['__call']

				local ext_args   = U.list_concat({expr.base}, args)
				local ext_arg_ts = U.list_concat({fun_type}, arg_ts)

				local rets = analyze_fun_call(expr, call_t, ext_args, ext_arg_ts, report_errors)
				D.assert( rets==nil or T.is_type_list(rets) )
				return rets
