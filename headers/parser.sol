-- Compiled from sol/parser.sol on 2013 Sep 27  14:05:17

return <0x0160d4b0>{
	-- Types:
	typedef AssignmentStatement = <0x01189230>{
	...
	typedef BooleanExpr = <0x0115f360>{
		ast_type: "BooleanExpr";
		tokens:   [Token];
		value:    bool;
		where:    string;
	};
	typedef BreakStatement = <0x01190b70>{
		ast_type: "BreakStatement";
		scope:    Scope?;
		tokens:   [Token];
		where:    string;
	};
	typedef CallExpr = <0x01161cb8>{
		arguments: [ExprNode];
		ast_type:  "CallExpr";
		base:      ExprNode;
		tokens:    [Token];
	...
	typedef CallStatement = <0x01189b30>{
	...
	typedef CastExpr = <0x01188408>{
		ast_type: "CastExpr";
		expr:     ExprNode;
		tokens:   [Token];
		where:    string;
	};
	typedef ClassDeclStatement = <0x0118b3d0>{
	...
	typedef ConstructorExpr = <0x011875e8>{
		ast_type:   "ConstructorExpr";
		entry_list: [ConstructorExprEntry];
		tokens:     [Token];
		where:      string;
	};
	typedef DoStatement = <0x0118d158>{
	...
	typedef DotsExpr = <0x011616c8>{
		ast_type: "DotsExpr";
		tokens:   [Token];
		where:    string;
	};
	typedef Eof = <0x01192618>{
		ast_type: "Eof";
		scope:    Scope?;
		tokens:   [Token];
		where:    string;
	};
	typedef ExprNode = <0x0115d830>{
		ast_type: ExprType;
		tokens:   [Token];
		where:    string;
	};
	...
	typedef ExternExpr = <0x01160000>{
		ast_type: "ExternExpr";
		tokens:   [Token];
		where:    string;
	};
	typedef FunctionDeclStatement = <0x01191160>{
		arguments:    [<0x01191980>{
		              		name: string;
		              		type: Type?;
		              	}];
	...
	typedef GenericForStatement = <0x0118d868>{
	...
	typedef GotoStatement = <0x0118fd50>{
	...
	typedef IfStatement = <0x0118c208>{
	...
	typedef LabelStatement = <0x0118f6a0>{
	...
	typedef LambdaFunctionExpr = <0x011857b8>{
		arguments:    [<0x01185a80>{
		              		name: string;
		              		type: Type?;
		              	}];
	...
	typedef NilExpr = <0x0115fa10>{
		ast_type: "NilExpr";
		tokens:   [Token];
		where:    string;
	};
	typedef Node = <0x0115c898>{
		ast_type: NodeType;
		tokens:   [Token];
		where:    string;
	};
	typedef NodeType = ExprType or StatType or "Statlist";
	typedef NumberExpr = <0x0115e600>{
		ast_type: "NumberExpr";
		tokens:   [Token];
		value:    string;
		where:    string;
	};
	typedef NumericForStatement = <0x0118e220>{
	...
	typedef ParenthesesExpr = <0x01187cf8>{
		ast_type: "ParenthesesExpr";
		inner:    ExprNode;
		tokens:   [Token];
		where:    string;
	};
	typedef RepeatStatement = <0x0118ee60>{
	...
	typedef ReturnStatement = <0x01190400>{
		arguments: [ExprNode];
	...
	typedef Statlist = <0x0115d0c0>{
		ast_type: "Statlist";
		body:     [StatNode];
		tokens:   [Token];
		where:    string;
	};
	typedef StringCallExpr = <0x01183e40>{
	...
	typedef StringExpr = <0x0115ecb0>{
		ast_type: "StringExpr";
		tokens:   [Token];
		value:    string;
		where:    string;
	};
	typedef TableCallExpr = <0x011835a0>{
	...
	typedef VarDeclareStatement = <0x0118a240>{
	...
	typedef WhileStatement = <0x0118c918>{
