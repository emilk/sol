-- Compiled from sol/parser.sol on 2013 Sep 28  09:52:38

return <0x011e91f0>{
	-- Types:
	typedef AssignmentStatement = <0x011bd0c8>{
	...
	typedef BooleanExpr = <0x011b4258>{
		ast_type: "BooleanExpr";
		tokens:   [Token];
		value:    bool;
		where:    string;
	};
	typedef BreakStatement = <0x011e5a50>{
		ast_type: "BreakStatement";
		scope:    Scope?;
		tokens:   [Token];
		where:    string;
	};
	typedef CallExpr = <0x011b6bb0>{
		arguments: [ExprNode];
		ast_type:  "CallExpr";
		base:      ExprNode;
		tokens:    [Token];
	...
	typedef CallStatement = <0x011bd9c8>{
	...
	typedef CastExpr = <0x011bc2a0>{
		ast_type: "CastExpr";
		expr:     ExprNode;
		tokens:   [Token];
		where:    string;
	};
	typedef ClassDeclStatement = <0x011bf268>{
	...
	typedef ConstructorExpr = <0x011bb480>{
		ast_type:   "ConstructorExpr";
		entry_list: [ConstructorExprEntry];
		tokens:     [Token];
		where:      string;
	};
	typedef DoStatement = <0x011c0ff0>{
	...
	typedef DotsExpr = <0x011b65c0>{
		ast_type: "DotsExpr";
		tokens:   [Token];
		where:    string;
	};
	typedef Eof = <0x011e74f8>{
		ast_type: "Eof";
		scope:    Scope?;
		tokens:   [Token];
		where:    string;
	};
	typedef ExprNode = <0x011b27b8>{
		ast_type: ExprType;
		tokens:   [Token];
		where:    string;
	};
	...
	typedef ExternExpr = <0x011b4ef8>{
		ast_type: "ExternExpr";
		tokens:   [Token];
		where:    string;
	};
	typedef FunctionDeclStatement = <0x011e6040>{
		arguments:    [<0x011e6860>{
		              		name: string;
		              		type: Type?;
		              	}];
	...
	typedef GenericForStatement = <0x011c1700>{
	...
	typedef GotoStatement = <0x011e4c30>{
	...
	typedef IfStatement = <0x011c00a0>{
	...
	typedef LabelStatement = <0x011e4580>{
	...
	typedef LambdaFunctionExpr = <0x011b9668>{
		arguments:    [<0x011b9930>{
		              		name: string;
		              		type: Type?;
		              	}];
	...
	typedef NilExpr = <0x011b4908>{
		ast_type: "NilExpr";
		tokens:   [Token];
		where:    string;
	};
	typedef Node = <0x011b17b8>{
		ast_type: NodeType;
		tokens:   [Token];
		where:    string;
	};
	typedef NodeType = ExprType or StatType or "Statlist";
	typedef NumberExpr = <0x011b34f8>{
		ast_type: "NumberExpr";
		tokens:   [Token];
		value:    string;
		where:    string;
	};
	typedef NumericForStatement = <0x011e3100>{
	...
	typedef ParenthesesExpr = <0x011bbb90>{
		ast_type: "ParenthesesExpr";
		inner:    ExprNode;
		tokens:   [Token];
		where:    string;
	};
	typedef RepeatStatement = <0x011e3d40>{
	...
	typedef ReturnStatement = <0x011e52e0>{
		arguments: [ExprNode];
	...
	typedef Statlist = <0x011b1fe0>{
		ast_type: "Statlist";
		body:     [StatNode];
		tokens:   [Token];
		where:    string;
	};
	typedef StringCallExpr = <0x011b7cf0>{
	...
	typedef StringExpr = <0x011b3ba8>{
		ast_type: "StringExpr";
		tokens:   [Token];
		value:    string;
		where:    string;
	};
	typedef TableCallExpr = <0x011b7450>{
	...
	typedef VarDeclareStatement = <0x011be0d8>{
	...
	typedef WhileStatement = <0x011c07b0>{
	...
	-- Members:
	LUA_SETTINGS: <0x01b50d18>{
	              	function_types: false;
	              	is_sol:         false;
	              	keywords:       {string};
	              	symbols:        {string};
	              };
	SOL_SETTINGS: <0x01b5d860>{
	              	function_types: true;
	              	is_sol:         true;
	              	keywords:       {string};
	              	symbols:        {string};
